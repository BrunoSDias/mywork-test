...
RSpec.describe TimetrackingsController, type: :controller do
  describe "GET new" do
    it "- verify if assign empty instance variable" do
      get :new
    ...
    it "- verify if render with not empty instance variable" do
  ...
  describe "POST create" do
    it "- verify if render new template with localization error" do
      post :create, :params => { :timetracking => { :latitude => 999999, :longitude => 999999, :checkin => 1560884347911} }
      expect(flash[:error]).to be_present
      expect(response).to render_template("new")
    end

    it "- verify if render new template sending id param" do
